...
export default function DetailView({ animeId, setView, favorites, toggleFavorite }: DetailViewProps) {
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-24 text-center space-y-4">
        ...
        <p className="text-sm text-gray-400 font-medium">{lang === "ja" ? "作品データを高画質で読み込み中..." : "Loading anime media data..."}</p>
      </div>
    );
  }
  ...
  if (!anime) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-24 text-center">
        <p className="text-gray-500">{lang === "ja" ? "作品情報が見つかりませんでした。" : "Anime details not found."}</p>
        <button
  ...
  const mainTitle = (lang === "en" ? anime.title?.english || anime.title?.userPreferred : anime.title?.native || anime.title?.userPreferred) || anime.title?.english || anime.title?.romaji || "Untitled";
  ...
  const affiliates = [
    {
      name: "U-NEXT",
      description: lang === "ja" 
        ? "31日間無料トライアル実施中！27万本以上が見放題。アニメ配信数も業界トップクラス。無料登録時に600円分のポイントが貰えて、最新作のレンタルにも使えます！"
        : "31-day free trial! Over 270,000 videos available for streaming with top-class anime collection.",
      cta: lang === "ja" ? "U-NEXTで31日間無料体験" : "Start 31-Day Free Trial on U-NEXT",
      ...
      badge: lang === "ja" ? "おすすめNo.1" : "Top Pick"
    },
    {
      name: "DMM TV",
      description: lang === "ja"
        ? "月額550円で19万本以上が見放題！新規登録なら30日間無料＆最大3ヶ月間550pt付与。アニメ最速先行配信が非常に多くコスパ最高のサービス。"
        : "Unlimited streaming for ¥550/month with 30 days free for new subscribers.",
      cta: lang === "ja" ? "DMM TVで30日間無料体験" : "Start 30-Day Free Trial on DMM TV",
      ...
      badge: lang === "ja" ? "コスパ最強" : "Best Value"
    },
    {
      name: "dアニメストア",
      description: lang === "ja"
        ? "アニメ専門配信サービスNo.1！月額550円で5,700作品以上のアニメが見放題。懐かしの名作から今期の地上波同時配信まで網羅。"
        : "Japan's #1 specialized anime streaming service with over 5,700 titles.",
      cta: lang === "ja" ? "dアニメストアで今すぐ見る" : "Watch on dAnime Store",
      ...
      badge: lang === "ja" ? "作品数No.1" : "Largest Selection"
    },
    {
      name: "ABEMA Premium",
      description: lang === "ja"
        ? "プレミアムプラン（月額960円）で新作アニメの地上波最速配信や限定コンテンツ、見逃し配信が広告なしですべて見放題に。"
        : "Ad-free streaming of latest TV anime broadcasts and exclusive titles.",
      cta: lang === "ja" ? "ABEMAで配信チェック" : "Check Streams on ABEMA",
      ...
      badge: lang === "ja" ? "地上波同時" : "Simulcast"
    },
    {
      name: "Amazon Prime Video",
      description: lang === "ja"
        ? "月額600円のプライム会員特典で追加料金なしで見放題！プライム配送特典などもコミコミで非常にお得です。"
        : "Watch included with Amazon Prime membership along with fast shipping benefits.",
      cta: lang === "ja" ? "Amazonプライムで検索" : "Search on Prime Video",
      ...
      badge: lang === "ja" ? "プライム特典" : "Prime Perk"
    },
  ];
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 space-y-8 font-sans">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span>{lang === "ja" ? "作品一覧へ戻る" : "Back to Anime List"}</span>
        </button>
        ...
        >
          ...
          <span>{copied ? (lang === "ja" ? "リンクをコピーしました" : "Link Copied!") : (lang === "ja" ? "この作品を共有" : "Share This Anime")}</span>
        </button>
      </div>
        ...
        <div className="relative z-10 px-6 pb-6 -mt-20 sm:px-10 sm:pb-8 flex flex-col sm:flex-row gap-6 items-start">
          ...
          <div className="flex-grow space-y-4 self-stretch flex flex-col justify-end pt-4 sm:pt-0">
            <div>
              <div className="flex items-center space-x-2 mb-1">
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-rose-50 text-rose-600 border border-rose-100">
                  ...
                  {lang === "ja" ? "注目アニメ" : "Featured Anime"}
                </span>
                {anime.startDate?.year && (
                  <span className="text-xs text-gray-400 font-semibold">{anime.startDate.year}{lang === "ja" ? "年作品" : ""}</span>
                )}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center space-x-1.5 rounded-xl bg-amber-50 px-3 py-1.5 text-xs font-bold text-amber-800 border border-amber-200/60 shadow-sm">
                <Star className="h-4 w-4 fill-amber-400 text-amber-400" />
                <span className="text-[11px] font-bold text-amber-900">{lang === "ja" ? "診断ユーザー満足度" : "User Rating"}</span>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              >
                ...
                <span>{isFav ? (lang === "ja" ? "お気に入り追加済み" : "In Favorites") : (lang === "ja" ? "お気に入りに追加" : "Add to Favorites")}</span>
              </button>

              <a
                href={`https://www.google.com/search?q=${encodeURIComponent(mainTitle + " anime")}`}
                target="_blank"
              ...
              >
                ...
                <span>{lang === "ja" ? "Googleで最新情報を検索" : "Search Info on Google"}</span>
      ...
      <div className="flex border-b border-gray-200 gap-2 overflow-x-auto scrollbar-none">
        <button
          ...
          className={`flex items-center space-x-2 py-3 px-6 text-sm font-bold border-b-2 transition-all shrink-0 ${
            activeTab === "overview"
        ...
        >
          ...
          <span>{lang === "ja" ? "概要・作品あらすじ" : "Synopsis & Overview"}</span>
        </button>
        ...
        >
          ...
          <span>{lang === "ja" ? "作品スペック・データ" : "Anime Details"}</span>
        </button>
        ...
        >
          ...
          <span>{lang === "ja" ? "配信視聴サービス" : "Streaming Services"}</span>
        </button>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 sm:p-8 shadow-md space-y-4">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                <h3 className="text-base font-extrabold text-gray-900 flex items-center space-x-2">
                  ...
                  <span>{lang === "ja" ? "ストーリー概要・あらすじ" : "Synopsis"}</span>
                </h3>
                <span className="text-xs text-gray-400 font-medium">{lang === "ja" ? "公式メディアデータ" : "Official Media Data"}</span>
              </div>
            ...
            {highlightPart && (
              <div className="rounded-2xl border border-rose-100 bg-gradient-to-br from-rose-50/60 to-purple-50/40 p-6 sm:p-8 shadow-sm space-y-4">
                <h3 className="text-base font-extrabold text-rose-950 flex items-center space-x-2">
                  ...
                  <span>{lang === "ja" ? "この作品の見どころ・魅力ポイント" : "Highlights & Features"}</span>
                </h3>
            ...
            {anime.genres && anime.genres.length > 0 && (
              <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-3">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">{lang === "ja" ? "作品のジャンル・属性" : "Genres & Tags"}</h4>
                <div className="flex flex-wrap gap-2">
                  {anime.genres.map((g, idx) => (
                    ...
                    >
                      ...
                      <span>{lang === "ja" ? translateGenreToJapanese(g) : g}</span>
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 sm:p-8 shadow-md space-y-6">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                <h3 className="text-base font-extrabold text-gray-900 flex items-center space-x-2">
                  ...
                  <span>{lang === "ja" ? "基本データ・スペック一覧" : "Media Specifications"}</span>
                </h3>
              </div>
              ...
              <dl className="grid grid-cols-1 gap-y-4 gap-x-8 sm:grid-cols-2 text-sm">
                <div className="p-3 rounded-xl bg-gray-50 border border-gray-100 flex justify-between items-center">
                  <dt className="font-semibold text-gray-500 text-xs">{lang === "ja" ? "邦題（正式名称）" : "Native Title"}</dt>
                  <dd className="text-gray-900 font-bold text-sm text-right">{anime.title?.native || mainTitle}</dd>
                </div>

                <div className="p-3 rounded-xl bg-gray-50 border border-gray-100 flex justify-between items-center">
                  <dt className="font-semibold text-gray-500 text-xs">{lang === "ja" ? "英題 / ローマ字" : "English Title"}</dt>
                ...
                <div className="p-3 rounded-xl bg-gray-50 border border-gray-100 flex justify-between items-center">
                  <dt className="font-semibold text-gray-500 text-xs">{lang === "ja" ? "放送開始日" : "Release Date"}</dt>
                  <dd className="text-gray-900 font-bold text-sm text-right">
                    {anime.startDate?.year
                      ? (lang === "ja" ? `${anime.startDate.year}年 ${anime.startDate.month || 1}月 ${anime.startDate.day || 1}日` : `${anime.startDate.year}-${anime.startDate.month || 1}-${anime.startDate.day || 1}`)
                      : "TBA"}
                  </dd>
                </div>

                <div className="p-3 rounded-xl bg-gray-50 border border-gray-100 flex justify-between items-center">
                  <dt className="font-semibold text-gray-500 text-xs">{lang === "ja" ? "アニメーション制作会社" : "Animation Studio"}</dt>
                  <dd className="text-gray-900 font-bold text-sm text-right">{anime.studios?.nodes?.[0]?.name || (lang === "ja" ? "情報更新中" : "TBA")}</dd>
                </div>

                <div className="p-3 rounded-xl bg-gray-50 border border-gray-100 flex justify-between items-center">
                  <dt className="font-semibold text-gray-500 text-xs">{lang === "ja" ? "診断ユーザー満足度" : "User Rating"}</dt>
        ...
        {activeTab === "streaming" && (
          <div className="space-y-6">
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-2">
              <h3 className="text-base font-extrabold text-gray-900">{lang === "ja" ? "配信中の公式サービス一覧" : "Official Streaming Platforms"}</h3>
              <p className="text-xs text-gray-500 leading-relaxed">
                {lang === "ja" 
                  ? `『${mainTitle}』を安全・高画質でお楽しみいただける公式動画配信サービスです。初回登録の無料トライアルを活用してご視聴いただけます。`
                  : `Official streaming platforms where you can safely watch "${mainTitle}" with free trial options.`}
              </p>
            </div>
      ...
      {/* Footnote */}
      <div className="flex items-center space-x-2 text-[11px] text-gray-400 justify-center pt-4 text-center">
        <ShieldCheck className="h-4 w-4 text-emerald-500 shrink-0" />
        <span>{lang === "ja" ? "当サイトは合法かつ安全に作品情報および配信情報を提供しています。" : "This platform provides legal, safe, and official anime media information."}</span>
      </div>
    </div>
  );
}
