import { AnimeMedia } from "../types";

export const FALLBACK_POPULAR_ANIME: AnimeMedia[] = [
  {
    id: 154587,
    title: {
      native: "葬送のフリーレン",
      romaji: "Sousou no Frieren",
      english: "Frieren: Beyond Journey's End",
      userPreferred: "葬送のフリーレン",
    },
    description: `【ストーリー概要】
魔王を倒し王都へ凱旋した勇者ヒンメル、僧侶ハイター、戦士アイゼン、そしてエルフの魔法使いフリーレンの4人一行。10年に及ぶ冒険の末に世界に平和をもたらした彼らは、50年に一度の「半世紀流星（エーラ流星）」を共に鑑賞し、再会を約束してそれぞれの道を歩み出します。

しかし、千年以上を生きるエルフのフリーレンにとって、人間の寿命の長さはほんの一瞬。50年後に再会した時、かつての仲間ヒンメルは老い、やがて息を引き取ります。彼の葬儀の場で、これまで人間の短い命に深く関わろうとしてこなかった自分を省み、泣き崩れるフリーレン。「もっと人間を知っておけばよかった」——後悔を胸に抱いた彼女は、人間の心を知るための新たな旅へと踏み出します。

【作品のみどころ・特徴】
・「冒険の終わり」から始まる切なくも温かいファンタジー後日譚ドラマ
・圧倒的な映像美と繊細な演出（アニメーション制作：マッドハウス）
・ヒンメルの言葉や仲間との思い出が、現在の旅路と美しく交錯する緻密なストーリー構成
・圧倒的な魔法戦のアクション描写と、静かで情緒あふれる日常表現のコントラスト`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx154587-82gsc876mK4A.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx154587-82gsc876mK4A.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b154587-82gsc876mK4A.jpg",
      color: "#e0e7ff",
    },
    bannerImage: "https://s4.anilist.co/file/anilistcdn/media/anime/banner/154587-k2G8M8O25V5K.jpg",
    startDate: { year: 2023, month: 9, day: 29 },
    episodes: 28,
    genres: ["Fantasy", "Adventure", "Drama"],
    averageScore: 93,
    studios: { nodes: [{ name: "マッドハウス" }] },
  },
  {
    id: 150672,
    title: {
      native: "【推しの子】",
      romaji: "Oshi no Ko",
      english: "OSHI NO KO",
      userPreferred: "【推しの子】",
    },
    description: `【ストーリー概要】
地方都市で産婦人科医として働くゴローの前に、彼の「推し」アイドルである「B小町」のアイが突如姿を現します。彼女はある大きな秘密を抱えており、ゴローは主治医として彼女を支えることを決意します。しかし、アイの出産直前、ゴローは何者かの手によって命を奪われてしまいます。

再び目を覚ました時、ゴローはなんとアイの産んだ双子の赤ちゃん「アクア」として転生していました。もう一人の双子の妹「ルビー」もまた、前世の記憶を持った転生者。憧れのアイの子供として幸福な幼少期を過ごす二人でしたが、ある日絶望的な悲劇が二人を襲います。成長したアクアは、事件の陰に潜む真犯人を追い詰めるため、欲望と策略が渦巻く芸能界の暗部へと足を踏み入れます。

【作品のみどころ・特徴】
・アイドル・演劇・テレビ業界の「光と影」をリアルかつ多角的に描くサスペンス
・原作：赤坂アカ×横槍メンゴのタッグが贈る圧倒的な展開と巧みな伏線回収
・YOASOBIによる主題歌「アイドル」的世界的現象と、動画工房による圧巻の作画・ライブ演出`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx150672-P2gM0x52j445.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx150672-P2gM0x52j445.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b150672-P2gM0x52j445.jpg",
      color: "#ec4899",
    },
    startDate: { year: 2023, month: 4, day: 12 },
    episodes: 11,
    genres: ["Drama", "Mystery", "Supernatural"],
    averageScore: 89,
    studios: { nodes: [{ name: "動画工房" }] },
  },
  {
    id: 161645,
    title: {
      native: "薬屋のひとりごと",
      romaji: "Kusuriya no Hitorigoto",
      english: "The Apothecary Diaries",
      userPreferred: "薬屋のひとりごと",
    },
    description: `【ストーリー概要】
花街で薬師として育てられた少女・猫猫（マオマオ）は、薬草採取の最中に人さらいに遭い、帝の妃たちが暮らす「後宮」に下女として売り払われてしまいます。目立たず年期が明けるのを待つつもりだった猫猫ですが、帝の御子たちが次々と不審な病に倒れる事件が発生。

好奇心と薬学の知識から、事件の真相「毒」の存在を突き止めた猫猫は、暗号のような警告文を残します。それに気づいた美貌の宦官・壬氏（ジンシ）によって能力を見抜かれ、猫猫は帝の寵妃の毒見役に抜擢されることに。薬と毒の膨大な知識、持ち前の観察眼とクールな推理力で、絢爛豪華な宮中の陰に隠された愛憎、陰謀、そして難事件を次々と解き明かしていきます。

【作品のみどころ・特徴】
・薬学知識と鋭い観察眼で謎を解く、爽快な本格中華風後宮ミステリー
・猫猫（CV: 悠木碧）と美貌の宦官・壬氏（CV: 大塚剛央）の独特で軽妙な掛け合い
・色彩豊かな美しい宮廷の美意識と、後宮を取り巻く人間ドラマの重厚な描写`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx161645-a764P9727M1R.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx161645-a764P9727M1R.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b161645-a764P9727M1R.jpg",
      color: "#10b981",
    },
    startDate: { year: 2023, month: 10, day: 22 },
    episodes: 24,
    genres: ["Drama", "Mystery"],
    averageScore: 88,
    studios: { nodes: [{ name: "TOHO animation STUDIO / OLM" }] },
  },
  {
    id: 101922,
    title: {
      native: "鬼滅の刃",
      romaji: "Kimetsu no Yaiba",
      english: "Demon Slayer: Kimetsu no Yaiba",
      userPreferred: "鬼滅の刃",
    },
    description: `【ストーリー概要】
時は大正日本。炭を売る心優しき少年・竈門炭治郎は、家族を人喰い鬼に惨殺され、唯一生き残った妹の禰豆子も凶悪な鬼に変貌してしまうという悲劇に見舞われます。絶望の中、鬼殺隊の剣士・冨岡義勇と出会った炭治郎は、妹を人間に戻し、家族を奪った鬼の始祖・鬼舞辻無惨を倒すため、鬼狩りの組織「鬼殺隊」へ入隊を決意します。

厳しい修行を乗り越え、命がけの「最終選別」を突破した炭治郎は、同期の我妻善逸や嘴平伊之助と共に、各地で人々を脅かす凄惨な鬼たちとの死闘に挑みます。家族の絆、仲間の愛、そして倒される鬼たちの悲しい過去にも寄り添う炭治郎の優しさが、過酷な戦いの中で光を放ちます。

【作品のみどころ・特徴】
・ufotableによる世界最高峰のアニメーション表現・「全集中の呼吸」剣戟アクション
・和風の情緒あふれる音楽・美術と、胸を打つ人間ドラマ・家族愛
・個性豊かな柱（最高位剣士）たちや魅力あふれるキャラクター陣の活躍`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx101922-W3TraceB4Cze.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx101922-W3TraceB4Cze.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b101922-W3TraceB4Cze.jpg",
      color: "#f43f5e",
    },
    startDate: { year: 2019, month: 4, day: 6 },
    episodes: 26,
    genres: ["Action", "Fantasy", "Supernatural"],
    averageScore: 85,
    studios: { nodes: [{ name: "ufotable" }] },
  },
  {
    id: 113415,
    title: {
      native: "呪術廻戦",
      romaji: "Jujutsu Kaisen",
      english: "Jujutsu Kaisen",
      userPreferred: "呪術廻戦",
    },
    description: `【ストーリー概要】
驚異的な身体能力を持つ高校生・虎杖悠仁は、ごく普通の日常を送っていましたが、ある日、学校に眠る危険な呪物「両面宿儺の指」の封印が解かれたことで、凶悪な「呪霊」たちの襲撃に遭遇します。取り残された先輩方を救うため、自らその特級呪物を呑み込んだ虎杖は、最強の呪いの王「宿儺」を自身の肉体に宿すことになります。

本来なら即座に処刑される運命だった虎杖ですが、最強の呪術師・五条悟の計らいにより、東京都立呪術高等専門学校へと編入。「宿儺の指を全て集めてから死ぬ」という過酷な使命を背負い、伏黒恵や釘崎野薔薇ら仲間とともに、人間の負の感情から生まれる呪いとの壮絶な戦いへ身を投じていきます。

【作品のみどころ・特徴】
・MAPPAが手掛ける極上のスタイリッシュ・ハイスピードバトルアクション
・「領域展開」をはじめとする洗練された呪術バトルシステムと設定の深み
・五条悟をはじめ、圧倒的カリスマと魅力を備えた登場人物たち`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx113415-9A2292S1a76M.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx113415-9A2292S1a76M.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b113415-9A2292S1a76M.jpg",
      color: "#6366f1",
    },
    startDate: { year: 2020, month: 10, day: 3 },
    episodes: 24,
    genres: ["Action", "Supernatural", "Fantasy"],
    averageScore: 86,
    studios: { nodes: [{ name: "MAPPA" }] },
  },
  {
    id: 130003,
    title: {
      native: "ぼっち・ざ・ろっく！",
      romaji: "Bocchi the Rock!",
      english: "Bocchi the Rock!",
      userPreferred: "ぼっち・ざ・ろっく！",
    },
    description: `【ストーリー概要】
極度の人見知りでコミュ障な少女・後藤ひとり（通称：ぼっちちゃん）。中学時代、テレビで流れたロックバンドのインタビューをきっかけに「自分もギターを弾ければ人気者になれるかも」と思い立ち、毎日押し入れで1日6時間の猛練習を重ね、動画配信サイトで「ギターヒーロー」として絶賛されるほどの凄腕ギタリストへと成長します。

しかし、高校生になっても友達一人作れずバンドを組む夢も叶わぬまま、公園のベンチでギターケースを抱えて途方に暮れていたところ、ドラムを担当する元気溢れる少女・伊地知虹夏から突然の声をかけられます。欠員が出た彼女のバンド「結束バンド」に急遽サポートとして参加することになったひとりは、ベースの山田リョウ、後に加入するボーカル＆ギターの喜多郁代と共に、ライブハウス「STARRY」を中心にバンド活動を開始します。

【作品のみどころ・特徴】
・コミュ障あるあるの絶妙な共感と、狂気すら感じる独創的なギャグ演出（実写・粘土アニメ・顔芸）
・「結束バンド」による本格的なロックサウンドと、下北沢のリアルなライブハウス文化の丁寧な描写
・陰キャ少女が音楽と仲間を通じて少しずつ自己表現と「一歩」を踏み出していく青春成長劇`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx130003-k2F7J9kM3R9X.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx130003-k2F7J9kM3R9X.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b130003-k2F7J9kM3R9X.jpg",
      color: "#f59e0b",
    },
    startDate: { year: 2022, month: 10, day: 9 },
    episodes: 12,
    genres: ["Comedy", "Music", "Slice of Life"],
    averageScore: 88,
    studios: { nodes: [{ name: "CloverWorks" }] },
  },
  {
    id: 140960,
    title: {
      native: "SPY×FAMILY",
      romaji: "SPY x FAMILY",
      english: "SPY x FAMILY",
      userPreferred: "SPY×FAMILY",
    },
    description: `【ストーリー概要】
東西の冷戦状態が続く苛烈な時代。西国の凄腕スパイ〈黄昏（たそがれ）〉は、東国の危険な政治家デズモンドの動向を探るため、極秘任務オペレーション〈梟（ストリクス）〉を命じられます。その内容は「一週間以内に家族を作り、デズモンドの息子が通う名門名門名門イーデン校の懇親会に潜入せよ」という無茶なものでした。

精神科医ロイド・フォージャーに扮した彼は、孤児院で人の心が読める超能力少女「アーニャ」を引き取り、さらに殺し屋という裏の顔を持つ女性「ヨル」と出会い、利害の一致から「仮初めの家族」を結成します。父はスパイ、母は殺し屋、娘は超能力者。お互いに素性を隠しながら、受験、学校生活、世界平和を守る危機に挑む不器用で温かいホームコメディが幕を開けます。

【作品のみどころ・特徴】
・表情豊かなアーニャの可愛らしさと愛されキャラ、スリリングな裏稼業アクションの完璧な融合
・「偽りの家族」が本当の絆と温もりを育んでいく心温まるストーリー展開
・WIT STUDIO×CloverWorksの豪華共同制作によるスタイリッシュな美術とハイレベルな作画`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx140960-aR8m8A7G0M11.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx140960-aR8m8A7G0M11.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b140960-aR8m8A7G0M11.jpg",
      color: "#06b6d4",
    },
    startDate: { year: 2022, month: 4, day: 9 },
    episodes: 12,
    genres: ["Action", "Comedy", "Slice of Life"],
    averageScore: 85,
    studios: { nodes: [{ name: "WIT STUDIO / CloverWorks" }] },
  },
  {
    id: 16498,
    title: {
      native: "進撃の巨人",
      romaji: "Shingeki no Kyojin",
      english: "Attack on Titan",
      userPreferred: "進撃の巨人",
    },
    description: `【ストーリー概要】
圧倒的な力を持つ巨大な「巨人」たちが全てを支配する絶望の世界。生き残った人類は「マリア」「ローゼ」「シーナ」と呼ばれる三重の巨大な防壁を築き、その内側で100年の平和を貪っていました。しかしある日、壁を超える50m超の「超大型巨人」と「鎧の巨人」が出現。長年守られてきた壁が破られ、無数の巨人が街へ雪崩れ込みます。

目の前で母親を巨人に喰われた少年エレン・イェーガーは、「一匹残らず害滅してやる」と深い仇恨を誓い、幼馴染のミカサ・アルレルト、アルミン・アルレルトと共に壁外調査を担う「調査兵団」へ志願。人類の生存を賭けた凄惨な戦いの中で、エレン自らが「巨人化」する謎の能力を発現させたことで、世界の真実と巨人の正体、そして壁の外に隠されたあまりにも過酷な世界の歴史と真実が徐々に解き明かされていきます。

【作品のみどころ・特徴】
・緻密に張り巡らされた壮大な伏線と、世界観の概念を覆すあまりに衝撃的なストーリー展開
・立体機動装置によるスピード感あふれる縦横無尽の立体戦闘アクション表現
・澤野弘之・KOHTA YAMAMOTOが手掛ける壮大で荘厳な劇伴音楽と重厚な人間ドラマ`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx16498-C6P8M82S2K9M.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx16498-C6P8M82S2K9M.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b16498-C6P8M82S2K9M.jpg",
      color: "#71717a",
    },
    startDate: { year: 2013, month: 4, day: 7 },
    episodes: 25,
    genres: ["Action", "Drama", "Fantasy", "Mystery"],
    averageScore: 85,
    studios: { nodes: [{ name: "WIT STUDIO / MAPPA" }] },
  },
  {
    id: 127230,
    title: {
      native: "チェンソーマン",
      romaji: "Chainsaw Man",
      english: "Chainsaw Man",
      userPreferred: "チェンソーマン",
    },
    description: `【ストーリー概要】
『チェンソーの悪魔』ポチタと共に、デビルハンターとして極貧の生活を送る少年デンジ。親が遺した多額の借金を返済するため、ヤクザに酷使されながら劣悪な環境で死と隣り合わせの日々を過ごしていましたが、悪魔の裏切りによって残虐に殺害されてしまいます。

薄れる意識の中、ポチタは自らの心臓をデンジに譲り渡し、「私の心臓をあげる。代わりにデンジの夢を見せて」と契りを交わします。こうして頭部と両腕からチェンソーを出現させる『チェンソーマン』として蘇ったデンジは、現場に駆けつけた公安のデビルハンター・マキマに拾われ、公安対魔特異4課に所属。「食パンにジャムを塗って食べる」「女の子と抱き合う」といった等身大の欲望を胸に、凶悪な悪魔たちとの凄惨な死闘へ身を投じます。

【作品のみどころ・特徴】
・クエンティン・タランティーノ等の映画的オマージュを散りばめたMAPPAの重厚かつシネマティックな演出
・毎話ごとにエンディング楽曲＆映像が変わる前代未聞の豪華アーティストコラボレーション
・既存のヒーロー像を覆すデンジの生々しい欲望と、ダークで不条理な世界観の圧倒的インパクト`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx127230-22D80S31c51M.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx127230-22D80S31c51M.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b127230-22D80S31c51M.jpg",
      color: "#ef4444",
    },
    startDate: { year: 2022, month: 10, day: 12 },
    episodes: 12,
    genres: ["Action", "Supernatural", "Drama"],
    averageScore: 84,
    studios: { nodes: [{ name: "MAPPA" }] },
  },
  {
    id: 20464,
    title: {
      native: "ハイキュー!!",
      romaji: "Haikyuu!!",
      english: "Haikyu!!",
      userPreferred: "ハイキュー!!",
    },
    description: `【ストーリー概要】
ふとしたきっかけでTVで見た「小さな巨人」に憧れ、バレーボールを始めた小柄な少年・日向翔陽。中学最初で最後の公式戦で、圧倒的センスを持つ「コート上の王様」影山飛雄率いる強豪校に大敗し、雪辱を誓って憧れの烏野高校バレー部に入部します。

しかし、体育館の扉を開けると、そこには宿敵であったはずの影山飛雄の姿が。最初は反発し合う二人でしたが、日向の驚異的な跳躍力・身体能力と、影山の神業のような精密セッティングが組み合わさることで、敵のブロックを一瞬で置き去りにする伝説の「変人速攻」が開花。「飛べ」の垂れ幕を背に、かつての強豪「落ちた強豪、飛べない烏」と呼ばれた烏野高校が、ライバル校たちと競い合いながら全国の頂を目指します。

【作品のみどころ・特徴】
・Production I.Gによる臨場感・疾走感あふれる圧倒的なバレーボールアニメーション
・敵味方問わずすべてのキャラクターにドラマがあり、成長と葛藤が丁寧に描かれる熱血青春劇
・ボールの効果音やコートの響き、声優陣の熱演が一体となった胸を打つ試合描写`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx20464-927aO1M2b64a.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx20464-927aO1M2b64a.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b20464-927aO1M2b64a.jpg",
      color: "#f97316",
    },
    startDate: { year: 2014, month: 4, day: 6 },
    episodes: 25,
    genres: ["Sports", "Drama", "Comedy"],
    averageScore: 85,
    studios: { nodes: [{ name: "Production I.G" }] },
  },
  {
    id: 21459,
    title: {
      native: "僕のヒーローアカデミア",
      romaji: "Boku no Hero Academia",
      english: "My Hero Academia",
      userPreferred: "僕のヒーローアカデミア",
    },
    description: `【ストーリー概要】
世界総人口の約8割が超常能力「個性」を持って生まれる超人社会。個性を悪用する「敵（ヴィラン）」から人々を守る「ヒーロー」が憧れの職業となる中、主人公の少年・緑谷出久（デク）は生まれつき何の力も持たない「無個性」の落ちこぼれでした。

それでも幼少期からの夢であるヒーローを諦めきれず、観察ノートを書きためていたデクは、ある日憧れの平和の象徴・No.1ヒーローのオールマイトと運命的な遭遇を果たします。自らの危険を顧みずに友人を救おうとしたデクの「ヒーローの本質」を見抜いたオールマイトは、自らの聖なる継承能力「ワン・フォー・オール」を譲り渡すことを決意。雄英高校ヒーロー科に入学したデクは、爆豪勝己や轟焦凍らクラスメイトと共に「更に向こうへ！Plus Ultra！」の精神で成長していきます。

【作品のみどころ・特徴】
・ボンズ（BONES）が手掛ける圧倒的な迫力とダイナミズムを誇るバトル作画表現
・「無個性」の少年が努力と仲間との絆で真のヒーローへと登りつめる王道胸熱ストーリー
・個性豊かなキャラクターたちの能力を活かした戦術と、思想がぶつかり合う重厚なドラマ`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx21459-Yv2M6M2Z83C5.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx21459-Yv2M6M2Z83C5.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b21459-Yv2M6M2Z83C5.jpg",
      color: "#22c55e",
    },
    startDate: { year: 2016, month: 4, day: 3 },
    episodes: 13,
    genres: ["Action", "Adventure"],
    averageScore: 78,
    studios: { nodes: [{ name: "ボンズ" }] },
  },
  {
    id: 132405,
    title: {
      native: "その着せ替え人形は恋をする",
      romaji: "Sono Bisque Doll wa Koi wo Suru",
      english: "My Dress-Up Darling",
      userPreferred: "その着せ替え人形は恋をする",
    },
    description: `【ストーリー概要】
伝統工芸「雛人形」の顔を作る頭師（かしらし）を目指す男子高校生・五条新菜（わかな）。幼少期のトラウマから周りに趣味を打ち明けられず、クラスでも孤立しがちな静かな日々を送っていました。

ある日、放課後の被服実習室で一人でミシンを使っていた新菜の元に、クラスの話題の中心で人気者の美少女ギャル・喜多川海夢（まりん）が飛び込んできます。実は大のコスプレ好きである海夢は、大好きなアニメキャラの衣装を作ろうとして苦戦していたところ、新菜の圧倒的な裁縫技術を目撃。「私のコスプレ衣装、作ってくれない！？」と熱望され、新菜は戸惑いながらも衣装作りを引き受けることに。モノづくりへのリスペクトと、純粋な情熱を通して2人の距離が縮まっていく胸キュンラブコメディ。

【作品のみどころ・特徴】
・CloverWorksによる超美麗で透明感あふれる作画と、海夢の表情豊かな可愛らしさ
・コスプレ衣装作りの工程や型紙採寸など、モノづくりの面白さと奥深さをリアルに描写
・オタク趣味への真っ直ぐな肯定とリスペクトが詰まった、爽やかでキュンとする青春ドラマ`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx132405-b01J3L0K274a.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx132405-b01J3L0K274a.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b132405-b01J3L0K274a.jpg",
      color: "#e879f9",
    },
    startDate: { year: 2022, month: 1, day: 9 },
    episodes: 12,
    genres: ["Comedy", "Romance", "Slice of Life"],
    averageScore: 82,
    studios: { nodes: [{ name: "CloverWorks" }] },
  },
  {
    id: 153288,
    title: {
      native: "怪獣8号",
      romaji: "Kaijuu 8-gou",
      english: "Kaiju No. 8",
      userPreferred: "怪獣8号",
    },
    description: `【ストーリー概要】
怪獣発生率が世界屈指の怪獣大国・日本。主人公の日比野カフカは、かつて幼馴染の亜白ミナと共に「怪獣を全滅させて、二人で防衛隊員になろう」と約束を交わしましたが、採用試験に落選し続け、32歳となった現在は怪獣の討伐痕跡を清掃する専門会社「モンスタースイーパー」の清掃員として働いていました。

ミナが防衛隊第3部隊の隊長として脚光を浴びる中、情けなさと悔しさを抱えるカフカの元に、防衛隊員を目指す情熱あふれる後輩・市川レノが入社してきます。彼の存在に背中を押され、一度は諦めた防衛隊への挑戦を再決意したカフカでしたが、その直後、謎の小型怪獣が口から侵入し、自らが「怪獣8号」として討伐対象の怪獣に変身する能力を得てしまいます。正体を隠しながら怪獣の力で防衛隊試験に挑む、熱きバトルの幕が開けます。

【作品のみどころ・特徴】
・Production I.G×スタジオカラーによる洗練された圧巻のバトル作画と怪獣デザイン
・32歳のアラフォー男が泥臭く夢を追いかける爽快でエモーショナルな熱血ストーリー
・防衛隊のスタイリッシュな装備・兵器と怪獣パワーによる破壊的アクションの爽快感`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx153288-G4mH04M16mX8.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx153288-G4mH04M16mX8.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b153288-G4mH04M16mX8.jpg",
      color: "#10b981",
    },
    startDate: { year: 2024, month: 4, day: 13 },
    episodes: 12,
    genres: ["Action", "Sci-Fi"],
    averageScore: 81,
    studios: { nodes: [{ name: "Production I.G" }] },
  },
  {
    id: 235,
    title: {
      native: "名探偵コナン",
      romaji: "Meitantei Conan",
      english: "Detective Conan",
      userPreferred: "名探偵コナン",
    },
    description: `【ストーリー概要】
数々の難事件を解決し「平成のホームズ」「東の高校生探偵」と謳われた工藤新一。幼馴染の毛利蘭と遊園地トロピカルランドへ遊びに行った際、怪しい黒づくめの男たちの密売現場を目撃します。観察に夢中になっていた新一は、背後から接近した別の男に襲われ、開発中の試作毒薬「APTX4869」を飲まされてしまいます。

薬の副作用によって命は助かったものの、体が小学1年生の姿まで縮んでしまった新一。正体がバレれば周囲に危害が及ぶと判断した彼は、「江戸川コナン」と名乗り、阿笠博士の発明品を駆使しながら、蘭の父親が営む毛利探偵事務所に居居候を開始。小五郎を眠らせて数多の難事件を次々と解決しながら、自らの体を元に戻す鍵を握る「黒の組織」の謎を追い続けます。

【作品のみどころ・特徴】
・緻密なトリックとロジックが冴え渡る本格推理ミステリーとラブコメディの黄金比
・黒の組織・FBI・公安警察（安室透）など重厚で複雑に交錯するサスペンスドラマ
・劇場版シリーズでも絶大な人気を誇る、ド派手なアクションシーンとキャラクターの魅力`,
    coverImage: {
      extraLarge: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx235-L68R78pS9M5R.jpg",
      large: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx235-L68R78pS9M5R.jpg",
      medium: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/b235-L68R78pS9M5R.jpg",
      color: "#2563eb",
    },
    startDate: { year: 1996, month: 1, day: 8 },
    episodes: 1100,
    genres: ["Adventure", "Comedy", "Mystery"],
    averageScore: 82,
    studios: { nodes: [{ name: "トムス・エンタテインメント" }] },
  },
];

export const GENRE_JAPANESE_MAP: { [key: string]: string } = {
  Action: "バトル・アクション",
  Adventure: "冒険・アドベンチャー",
  Comedy: "コメディ・ギャグ",
  Drama: "感動・ドラマ",
  Fantasy: "ファンタジー・異世界",
  Horror: "ホラー・怪異",
  "Mahou Shoujo": "魔法少女",
  Mecha: "ロボット・メカ",
  Music: "音楽・バンド",
  Mystery: "推理・ミステリー",
  Psychological: "心理・サイコ",
  Romance: "恋愛・ラブコメ",
  "Sci-Fi": "SF・サイバーパンク",
  "Slice of Life": "日常・ほのぼの",
  Sports: "スポーツ・熱血",
  Supernatural: "オカルト・能力",
  Thriller: "サスペンス・スリラー",
};

export function translateGenreToJapanese(genre: string): string {
  return GENRE_JAPANESE_MAP[genre] || genre;
}
