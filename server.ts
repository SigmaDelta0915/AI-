...
これらを詳細に分析し、ユーザーのアニメの好みに合わせた「診断タイプ（オリジナルの二つ名）」と「詳細な性格分析」を生成してください。
...
【結果作成の重要ガイドライン】
1. 毎回同じ定番アニメ（進撃の巨人やフリーレン等）ばかりに偏らないでください。ユーザーの回答やスコア構成（恋愛、スポーツ、日常・コメディ、感動・ドラマ、SF・近未来、ミステリー、異世界、音楽など）を鋭く汲み取り、ジャンルに特化した多様で魅力的なアニメ（隠れた名作や様々な年代の名作含む）を選定してください。
2. 推薦理由は、ユーザーの休日スタイル、好みの主人公像、求めるテンポなどの具体的な回答と結びつけて、非常に説得力がある魅力的な文章（150〜200文字程度）にしてください。
3. 指定されたJSON構造（typeName, typeDescription, keyTraits, recommendations）に厳密に従い、有効なJSONのみを返却してください。`;
